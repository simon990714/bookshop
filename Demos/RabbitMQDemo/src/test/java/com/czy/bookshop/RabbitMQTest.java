...
public class RabbitMQTest {
    ...
    @Test
    void deadLetterTest() throws InterruptedException {
        User user1 = new User("张三", "123", new Date());
        User user2 = new User("李四", "321", new Date());
        //消息只能是String, byte[] and Serializable payloads，只要监听的时候取出该类
        rabbitTemplate.convertAndSend("normalExchange", "normalKey", JSONUtils.createJson(user1));
        System.out.println("用户1发送时间" + new Date());
        TimeUnit.SECONDS.sleep(5L);
        rabbitTemplate.convertAndSend("normalExchange", "normalKey", JSONUtils.createJson(user2));
        System.out.println("用户2发送时间" + new Date());
